...
function visaavgangar(siteId) {
    ...
    fetch(url)
        .then((resp) => resp.json())
        .then(function (data) {
            let departures = data.ResponseData.Trams;
            return departures.map(function (departure) {
                ...
                span.innerHTML = departure.GroupOfLine + " : " + departure.LineNumber + " : " + departure.Destination + " : " + departure.DisplayTime + " : " + devtext;
                append(li, img);
                append(li, span);
                append(ul, li);
            })
